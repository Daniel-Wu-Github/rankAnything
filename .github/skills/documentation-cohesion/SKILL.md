...
- Use cross-references to keep updates synchronized (e.g., "See the relevant decision note for the prerequisite detail")
...
### Example: Adding a Missing Prerequisite Requirement

**Bad (bolted on):** Create a new "Additional Requirements" section at the top of the document.

**Good (integrated):** 
1. Find the decision note.
2. Update the "Planned files" bullet to show the first change enables the required prerequisite.
3. Update the "Planning rule" to state the prerequisite must be completed before creating the resource.
4. Go to the relevant taskboard section. Update the checklist with `**Mandatory:** Include the prerequisite step.`
5. Update the "Done when" criteria to confirm the prerequisite is explicitly completed.
6. In the prompt, add the prerequisite to the mandatory requirements list.
...
2. **The Wishy-Washy Prompt:** "Consider adding a trigger" or "You might want to enable the prerequisite." Instead: "**Mandatory:** Include the prerequisite step." or "Create the required hook before enabling the feature."
