...
// Template JSON is injected into a <script type="application/json"> block, so
// a literal "</script>" anywhere in it would close the tag early and let the
// rest be parsed as markup. Templates are curated today, but this makes the
// injection safe by construction rather than by trusting future content.
...
// 1c. Static data snapshots (e.g. ADP, generated by scripts/fetch-adp.mjs) —
// served at /data/<file>. Not regenerated by this build; optional, since a
// fresh checkout won't have run the fetch script yet.
if (existsSync(join(here, "data"))) {
  cpSync(join(here, "data"), join(dist, "data"), { recursive: true });
}
